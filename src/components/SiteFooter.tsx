...
export function SiteFooter() {
  ...
  return (
    <footer className="border-t border-parichay-footer-border bg-parichay-footer">
...
function FooterLink({ to, children }: { to: string; children: React.ReactNode }) {
  return (
    <Link
      to={to}
      className="inline-flex min-h-11 items-center justify-center px-1 sm:px-2 transition-colors duration-200 ease-[var(--ease-out-expo)] hover:text-parichay-footer-text-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-parichay-footer-focus focus-visible:ring-offset-2 focus-visible:ring-offset-parichay-footer"
    >
      {children}
    </Link>
  );
